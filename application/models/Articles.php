...
class Articles extends Zend_Db_Table
{
	...
	public function getBySlug($slug, $format = 0)
	{
		...
		if($format)
		{
			if(substr_count($item->text, "<!-- pagebreak -->"))
			{
	...
	{
		$db = Zend_Registry::get("db");
		$query = "SELECT * FROM pros_article WHERE category = '$category' AND published = 1 ORDER BY ordering";
		if($count != 0)
			$query .= " LIMIT 0,$count";
		$stmt = $db->query($query);
		$stmt->setFetchMode(Zend_Db::FETCH_OBJ);
	...
	{
		$db = Zend_Registry::get("db");
		return $db->fetchOne("SELECT COUNT(slug) FROM pros_article WHERE category = '$category'");
	}
