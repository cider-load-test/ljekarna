...
class KontaktController extends Zend_Controller_Action
{
	...
	public function posalji2Action()
	{
		...
		if($this->getRequest()->isPost())
		{
			...
			if($ime == "" || $email == "" || $clanak == "")
			{
				$this->view->result = "<h1>Greška =(</h1><p>Potrebno je ispuniti sva polja.</p>";
			}
			else
			{
				$body = "Postovani, osoba koja se predstavila kao $ime, salje Vam sljedeci clanak sa stranice http://www.prospekta.net\n"
					  . "Clanak u cijelosti procitajte na http://www.prospekta.net/article/" . $slug . "\n\n"
				...
				{
					$mail->send();
					$this->view->result = "<h1>Potvrda</h1><p>E-mail je uspješno poslan. Zahvaljujemo.</p>";
				}
				catch (Exception $ex)
				{
		...
		{
			$this->view->result = "<h1>Greška =(</h1><p>Došlo je do greške. Molimo da probate kasnije</a></p>";
		}
	}
	...
	public function prijave2Action()
	{
		...
		if($this->getRequest()->isPost())
		{
			...
			{
				$this->view->result = "<h1>Greška =(</h1><p>Potrebno je ispuniti polja ime, prezime te e-mail.</p>";
			}
			else
			{
				$body = "Online prijava:\n\n"
